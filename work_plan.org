...
:END:
- [ ] extract gid from get skill to graph.
...
- [-] 加一个loop detection算法，现在只做了direct cycle detection algorithm.
  ...
  - [ ] it's too hard to detect cycles in the whole graph. Starting in a small categories.
  - [ ] don't add direct loop edges to a graph, find_cycles will only show such direct loop. save this graph.
  - [ ] remove direct cycle and full cycle at a node completely.
