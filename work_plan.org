...
:END:
- [ ] use [[http://www.cis.lmu.de/pub/phraseEmbedding.txt.bz2][phrase embedding]] as test.
- [X] take a look at cite space iii.
- [ ] increamental training finance pages based on google model.
- [ ] conpare the results from GS searching engine and word embedding.
- [ ] clustering categories by word embedding, osets, idea.
- [ ] test word2vec model from finance.
- [ ] cut paragraph to short sentences, then phrase.
- [X] phrase text8
- [ ] train phrasing sentences word2vec model.
