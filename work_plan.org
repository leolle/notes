...
:END:
- [ ] configuring deep learning hardware, operation system, software.
- [ ] test GPU server.
- [ ] test sets simularity, A-B=C-D?, A+B=
- [ ] visualize vocabulary embedding using t-SNE which project embedding vectors into 2-D surface from an proper perspective.
  need to batch words in frequency.
- [ ] train function names based on wiki pages on functions, models, and python/matlab/sas/cppreference manuals, function names and function descriptions, excel formula, VBA, VB, guass, whatever software which has a function dictionary and manual.
...
- [ ] LSA or LDA analysis, which will give a clustering of words on every topic.
