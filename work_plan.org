...
******* 金融搜索
****** 智能选股
爬取Google news, 雪球， 虎嗅， 微信上所有500支股票的文档，再进行sentiment analysis, 结果再排序，选最好的5只。
****** 知识数据分析
...
*** Knowledge Graph
**** TODO create wiki knowledge graph
...
- [ ]
**** TODO entity relationship
:LOGBOOK:
CLOCK: [2018-03-21 Wed 11:10]
...
- [ ] 先完成“投资”这一类语料的收集。
- [ ] 目标：按实体 实体 关系 语料内容的格式放入训练文件，以供模型训练。
- [ ] 思路：先通过网络爬虫从雪球，google news抓取“实体 投资”的内容，把抓取的内容（包括title, summary）通过NER识别出的结果。
- [ ] 丰富语料的思路：通过word2vec 相似词找到“投资”的相似词，如设立，增资，入股，收购，并购，换股;再找以上6个词的相似词。
下表为投资这一大类所包含的相似关系。
| 设立     | 增资     | 入股     | 收购     | 并购     | 换股 |
|----------+----------+----------+----------+----------+------|
...
- [ ] read paper *joint extraction of entities and relations*.
- [ ] read paper *Anomalies and Investor Sentiment*.
- [X] read_RMDB_table -> NLP_sentiment_analysis -> generate_sentiment_signal.
- [X] 情感分析指标的设计在GS上实现。
...
*** DONE Building the Wikipedia Knowledge Graph in Neo4j
...
- 目标学习：信息抽取
*** DONE 如何在NLP领域快速学会第一个技能？
CLOSED: [2018-02-09 Fri 19:00]
...
- [X] 查看Google Trende API.
** DONE GPU server hardware comparison
CLOSED: [2018-02-09 Fri 19:00]
...
** DONE Other's recommendation
CLOSED: [2018-02-09 Fri 19:01]
- [X] pytorch.
- [X] stuff from Zhupengjia, Liguofu.
