...
- [ ] read documentation.
*** Chinese wiki model.
- [ ] train financial fields model(58+ categories).
- [ ] use similarity distance to find the nearest category of target words.
...
- [ ] compare the training results with or without stopwords.
...
- 把word2vec应用在用户workflow使用序列上，根据用户使用app/func的顺序，把app/func看做单词，也是可以形成这样的序列数据，进而训练处每个app对应的向量。利用这个向量计算app之间的相似度，把真正内容相关的app聚合在一起，推荐给用户。
- 关系挖掘-哈工大论文.
