...
Starting from selecting one node at GS, the information system can recommend other connected keywords that maybe under the same topic from the opened website.
...
****** usecase 1 find task according to website:
******* use LDA to classify the current opened website content.
:LOGBOOK:
CLOCK: [2018-04-13 Fri 10:42]
:END:
or alternatively extract 1 or 2 document keyphrases from opened website, use this keyphrase as a tag for the website.
#+BEGIN_SRC python
...
******* find most similar task on GS:
1. find similar target task using current website theme.

****** TODO usecase 2 highlight entities according to GS task:
:LOGBOOK:
:END:
1. create a task named 查找动量指标, create requested want afterwards.
2. open a website(document) at the same time, extract entity related to (动量，指标） in that document via word embedding.
3. map these entities to GS.
4. [X] train word2vec model on opened document.
5. [X] find similar words of input.
6. [ ] join keywords and similar words.
#+BEGIN_SRC python
model.most_similar(u'指标')
#+END_SRC

#+RESULT:
:  [('市值', 0.36147886514663696),
:   ('模型', 0.3467492461204529),
:   ('ptt', 0.312774121761322),
:   ('节点', 0.2739245295524597),
:   ('网络', 0.27202028036117554),
:   ('专题报告', 0.25942254066467285),
:   ('DEA', 0.2574650049209595),
:   ('股价', 0.2555598020553589),
:   ('平均线', 0.25043344497680664),
:   ('MF', 0.24960537254810333)]
...
#+RESULT:
:  [(('A股', 0.2801433205604553),
:   ('CCICCI', 0.23448781669139862),
:   ('指标', 0.2320791333913803),
:   ('节点', 0.22942544519901276),
:   ('模型', 0.22912780940532684),
:   ('动力学', 0.2196020781993866),
:   ('ptt', 0.21833498775959015),
:   ('方法', 0.20321808755397797),
:   ('散户', 0.1981358826160431)]
...
#+RESULT:
:  [('距离', 0.2513788342475891),
:   ('平均线', 0.24599115550518036),
:   ('ptt', 0.22933638095855713),
:   ('收盘价', 0.22588738799095154),
:   ('指数', 0.2201143503189087),
:   ('速度感', 0.21861907839775085),
:   ('节点', 0.21831932663917542),
:   ('概率', 0.21310825645923615),
:   ('资金', 0.20784440636634827)]
