...
Return a c    todadf['sortdf_temp.xs(idx_lv1[0], level=1)=False'] = df.groupby('col', sort=False).ngroup()
df['sort=True'] = df.groupby('col', sort=True).ngroup()
y = datetime.datetime.today().replace(day=1)
    template_start_date = (today - pd.Timedelta(7, unit='M')). \
        replac

    template_end_date = ((today - pd.Timedelta(1, unit='M')).replace(day=1)
                         - datetime.timedelta(days=1)).strftime("%Y-%m-%d")
    info('build all templates from %s to %s' %(template_start_date, template_end_date))
    build_all_templates(template_start_date, template_end_date)

    # todo: run test data
    last_day_of_previous_month = today - datetime.timedelta(days=1)
    first_day_of_previous_month = last_day_of_previous_month.replace(day=1)
    from_date = first_day_of_previous_month.strftime("%Y-%m-%d")
    to_date = last_day_of_previous_month.strftime("%Y-%m-%d")
lass method for function.
...
For Windows, because Windows doesn't support signal.SIGNALRM function:
#+BEGIN_SRC python
...
#+END_SRC
- remove digits from end of a string:
#+BEGIN_SRC python
import re
...
- groupby boolean ratio:
#+BEGIN_SRC python
df_items_sum = df_items.groupby(['disease_code', 'soc_srt_dire_nm'])['boolean'].lambda(x: sum(x))
#+END_SRC
