...
- concatenate daterange from combined date and time :
#+BEGIN_SRC python
from datetime import datetime, time
ix_list = [pd.date_range(start=datetime.combine(x, time.fromisoformat('09:15:00')), periods=346, freq='min') for x in df['date'].unique()]
df_idx = pd.DatetimeIndex(np.unique(np.hstack(ix_list)))
#+END_SRC
...
# or
[j for i in ls_weight_users for j in i]
#+END_SRC
...
- element counter from array of list:
#+begin_src python
totals = Counter(i for i in list(itertools.chain.from_iterable(df_temp.values)))
#+end_src
...
- file path parameter:
#+begin_src python
fig.savefig(f'./png/{num}.{dropout}.{kernel_size}.{levels}.{nhid}'+'.png')
#+end_src
...
- split ls_codes
#+BEGIN_SRC python
ls_codes = np.array_split(ls_codes, np.ceil(len(ls_codes)/100))
df_daily = pd.DataFrame()
# 获取日线成交量
for i in ls_codes:
    df_temp = pro.daily(ts_code=','.join(i), start_date='20210101')
    df_daily = pd.concat([df_temp, df_daily])
df_daily['trade_date'] = pd.to_datetime(df_daily['trade_date'])
df_daily.sort_values('trade_date')
df_daily.iloc[:, 2:] = df_daily.iloc[:, 2:].apply(pd.to_numeric)
df_daily['code'] = df_daily['ts_code'].apply(lambda x: x[:-3])
#+END_SRC
...
- get proxies:
#+BEGIN_SRC python
with open(Path(r'F:\projects\wencai') / 'proxies.txt', 'r') as f:
    ls_ip_ports = f.readlines()
    ls_ip_ports = [x.strip() for x in ls_ip_ports]

def get_proxy():
    proxies = {
      "http": random.choice(ls_ip_ports)
    }
    return proxies
#+END_SRC
...
y = np.zeros((n_1_5, n_classes))
Y = np.random.choice(n_classes, n_1_5)

# assign with advanced indexing
x[np.arange(n_samples), J] = 1
y[np.arange(n_1_5), Y] = 1.5
q = np.concatenate((x, y), axis=0)
scores = []
for p in range(1000000):
    a = np.zeros((n_1_5+n_samples, n_classes))
    A = np.random.choice(n_classes, n_1_5+n_samples)
    a[np.arange(n_1_5+n_samples), A] = 1
    score = np.sum(a * q)
    scores.append(score)
#+END_SRC
...
- search for strings after a colon:
#+BEGIN_SRC python
:(.*)
#+END_SRC
...
for p in walk(Path('南怀瑾南禅七日讲座(MP3)')): 
    print(p)
#+END_SRC
...
#+BEGIN_SRC python
from pathlib import Path
get_current_working_dir = str(Path('.').absolute())
root_path = os.path.dirname(__file__) if '__file__' in dir() else os.getcwd()
#+END_SRC
...
output_file = Pathlib('file').with_name('new_' + p.stem).with_suffix('.mp3')
...
with gzip.open(new_data_head_path / 'pkl tick' / product / dump_file_name, 'wb', compresslevel=1) as fp:
...
- execute cmd command with variable parameter
#+BEGIN_SRC bash
...
- pandas apply if elif else:
#+BEGIN_SRC python
lambda x: x*10 if x<2 else (x**2 if x<4 else x+10)
#+END_SRC

- apply to every element in a dataframe:
#+BEGIN_SRC python
df_user_overview = df_user_overview.applymap(convert_unit)
#+END_SRC
...
datetime.datetime.today().replace(hour=13)
#+END_SRC
...
- filter by month:
#+BEGIN_SRC python
df_symbol_fold = df_symbol[df_symbol['Date'].apply(lambda x: x.strftime("%Y%m"))==fold]
#+END_SRC
...
- unique arrays from a list of arrays:
#+BEGIN_SRC python
arr_ins = np.unique(ls_ins, axis=0)
#+END_SRC
...
#+BEGIN_SRC python
 CREATE TABLE IF NOT EXISTS future.tssb_model  (
  date Date,    
  market String,     
  signal Nullable(Float),
  model String,
  ret  Nullable(Float)
 )ENGINE = ReplacingMergeTree() PRIMARY KEY (date, market, model) order by (date, market, model);
...
clickhouse_conn.execute("OPTIMIZE TABLE future.tbquant_daily FINAL")
#+END_SRC
...
FROM (    
    ...
    (        
        SELECT             
        lv2,     
        groupArray(code) AS arr_code,        
        groupArray(amount) AS arr_val,             
        arrayEnumerateDense(arr_val) AS dense_rank        
        FROM         
        (            
            SELECT 
                DISTINCT code, amount,                 
                lv2            
            FROM top_industry            
...
- n largest value and column name, idxmax(n)
#+BEGIN_SRC python
# for pivot table
pd.concat([df_buy_sell_ratio_industry.apply(lambda s: s.nlargest(3).index.tolist(), axis=1), \
          df_buy_sell_ratio_industry.apply(lambda s: s.nlargest(3).tolist(), axis=1)], axis=1)
# for raw dataframe
df_instrument_output.groupby('Date').apply(pd.DataFrame.nlargest, n=5, columns='COMM2')
...
- groupby combine columns with same columns:
#+BEGIN_SRC python
df_top_stocks['name'].to_frame().T.groupby(lambda x:x, axis=1).sum()
df_top_stocks.reset_index().drop('value', axis=1).groupby('index').agg(lambda col: col.tolist()).T
#+END_SRC
...
- drop duplicates in a list of arrays:
#+BEGIN_SRC python
a = [np.array([1,2,3]), np.array([1,2,3]), np.array([2,3,4])]
np.unique(a, axis=0)
#+END_SRC
...
print(random_date("2008/1/1 1:30:00", "2009/1/1 4:50:00", random.random()))
#+end_src
...
# or left join with b key is null
df_new = df_left.merge(df_right, on=['opendate', 'code'], suffixes=('', '_y'), indicator='i', how='outer').query('i == "left_only"').drop('i', 1).dropna(axis=1)
#+END_SRC
...
- access without keyerror, Pandas .loc without KeyError.
#+begin_src python
...
#+BEGIN_SRC python
fig, ax = plt.subplots()
ax.scatter(target[target['ret']>0][['STO_K_WEEK']],target[target['ret']>0][['NDH40']], marker='^')
ax.scatter(target[target['ret']<0][['STO_K_WEEK']],target[target['ret']<0][['NDH40']], marker='o')
...
# or
symbol = code + '.' + ct._code_to_symbol(code)[:2].upper()
#+END_SRC
...
# ignore labels printed when set_xticklabels called
...
fig = px.scatter_3d(target, x='CUBEFIT5', y='DOBV_20', z='ret',
                    color='ret')
fig.show()
...
- compare images:
#+BEGIN_SRC python
img_input = cv2.cvtColor(img_rgb[top+688:top+688+26, (left+265):(left+295),], cv2.COLOR_BGR2GRAY)
res = cv2.matchTemplate(img_input, img_gray, cv2.TM_CCOEFF_NORMED)
#+END_SRC
...
pytesseract.image_to_string(img_rgb[top+688:top+688+26, (left+110):right,], lang='chi_sim')
#+END_SRC
...
X = vectorizer.fit_transform(df.head(20).fillna(' ').values.tolist())
df_freq = pd.DataFrame(zip(vectorizer.get_feature_names(), X.toarray().sum(axis=0)))
df_freq.sort_values(1, ascending=False)
...
tfidf_model = models.TfidfModel(corpus_mod
el)
# 对语料生成tfidf
...
- pytorch ignore scientific mode:
